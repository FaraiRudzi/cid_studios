...
<head>
    <meta charset="UTF-8">
    <title>@yield('title', 'CID Studio Media Management')</title>
    ...
    {{-- Custom Tailwind Configuration for new colors --}}
    <script>
        tailwind.config = {
            darkMode: 'class',
            theme: {
    ...
    <aside id="sidebar" 
        ...
        <div class="p-3 border-b border-gray-200 dark:border-gray-700 shrink-0 bg-white">
            <div x-show="!isSidebarCollapsed" x-cloak>
                 <img src="{{ asset('images/logo.jpg') }}" alt="Logo" class="w-full h-auto max-h-24 object-contain">
            </div>
            <div x-show="isSidebarCollapsed" x-cloak>
                 <img src="{{ asset('images/logo.jpg') }}" alt="Logo" class="w-full h-auto p-1">
            </div>
        ...
        <div class="flex-grow flex flex-col bg-sidebar-gold dark:bg-sidebar-gold-dark">
            <nav class="flex-grow mt-4 space-y-2 px-3 overflow-y-auto">
                <a href="{{ route('admin.dashboard') }}" class="nav-link" x-bind:class="{ 'active': {{ request()->routeIs('admin.dashboard') ? 'true' : 'false' }} }">
                    <i data-feather="home"></i><span x-show="!isSidebarCollapsed" x-cloak>Dashboard</span>
                </a>
                <a href="{{ route('admin.stations.index') }}" class="nav-link" x-bind:class="{ 'active': {{ request()->routeIs('admin.stations.*') ? 'true' : 'false' }} }">
                    <i data-feather="map-pin"></i><span x-show="!isSidebarCollapsed" x-cloak>Stations</span>
                </a>
                <a href="{{ route('admin.cases.index') }}" class="nav-link" x-bind:class="{ 'active': {{ request()->routeIs('admin.cases.*') ? 'true' : 'false' }} }">
                    <i data-feather="folder"></i><span x-show="!isSidebarCollapsed" x-cloak>Cases</span>
                </a>
                <a href="{{ route('admin.photographers.index') }}" class="nav-link" x-bind:class="{ 'active': {{ request()->routeIs('admin.photographers.*') ? 'true' : 'false' }} }">
            ...
            <div class="flex items-center gap-4">
                ...
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" class="flex items-center gap-2">
                        <span class="hidden sm:inline">@auth {{ Auth::user()->name }} @else Guest @endauth</span>
...
{{-- CSS for sidebar and active links --}}
<style>
