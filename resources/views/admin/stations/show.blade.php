...
<div class="container mx-auto">
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md max-w-2xl mx-auto">
        ...
        <dl class="grid grid-cols-[auto,1fr] gap-x-4 gap-y-2 text-base">
            ...
            <dd class="text-gray-900 dark:text-gray-100">{{ $station->name }}</dd>

            <dt class="font-semibold text-gray-600 dark:text-gray-400">Created At:</dt>
            <dd class="text-gray-900 dark:text-gray-100">
                {{ $station->created_at ? $station->created_at->format('d M Y, H:i A') : 'N/A' }}
            </dd>
            
            <dt class="font-semibold text-gray-600 dark:text-gray-400">Last Updated:</dt>
            <dd class="text-gray-900 dark:text-gray-100">
                {{ $station->updated_at ? $station->updated_at->format('d M Y, H:i A') : 'N/A' }}
            </dd>
        </dl>
        ...
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.stations.edit', $station->id) }}" class="bg-yellow-500 text-white px-4 py-2 rounded-md hover:bg-yellow-600 font-semibold flex items-center gap-2">
                <i data-feather="edit" class="w-5 h-5"></i>
                Edit
            </a>
            
            {{-- THE FIX IS HERE: The old onclick is removed and replaced with a call to our new script --}}
            <form method="POST" action="{{ route('admin.stations.destroy', $station->id) }}" onsubmit="deleteConfirmation(event, this)">
                @csrf
                @method('DELETE')
